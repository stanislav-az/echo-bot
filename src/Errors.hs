...
tResponseErrorHandler
    ...
    -> ExceptT
           ...
           ()
tResponseErrorHandler (ResponseError status) =
    liftIO $ responseError Telegram status
tResponseErrorHandler (NoParse body) = liftIO $ parsingError Telegram body
tResponseErrorHandler (BadCallbackData badData) =
    liftIO $ badCallbackError Telegram badData

parsingError :: Bot -> String -> IO ()
parsingError bot body =
    (logError bot)
...
badCallbackError bot badData =
    (logError bot) $ "\tReceived bad callback data\n" ++ "\tIt was: " ++ badData
...
sResponseErrorHandler
    ...
    -> ExceptT
           ...
           ()
sResponseErrorHandler (ResponseError status) =
    liftIO $ responseError Slack status
sResponseErrorHandler (NoParse body) = liftIO $ parsingError Slack body
sResponseErrorHandler (BadCallbackData badData) =
    liftIO $ badCallbackError Slack badData
