...
sendLastMsgs = do
    ...
    checked <- liftIO $ runExceptT $ catchError (handleResponse response)
                                                tParsingErrorHandler
    ...
    runExceptT $ catchError (forM_ messages $ uncurry handleMessage)
                            tResponseErrorHandler
...
handleCallback (queryID, chatID, btnPressed) = do
    ...
    when dlog
        ...
        `T.append` "\tFor: "
        `T.append` (T.pack $ show chatID)
        `T.append` "\n"
        `T.append` "\tTo: "
        `T.append` (T.pack btnPressed)
...
makeSendMessage chatID msgText = do
    ...
    let req        = parseRequest_ $ "POST " ++ sr ++ "sendMessage"
        ...
        reqWithHeaders = setRequestHeaders
            ...
            req
    return $ setRequestBodyLBS (LB.fromStrict $ encodeUtf8 bodyText)
                               reqWithHeaders
...
makeAnswerCallbackQuery queryID btnPressed = do
    ...
    let req           = parseRequest_ $ "POST " ++ sr ++ "answerCallbackQuery"
        ...
        msgLBS =
            "You've choosen to repeat messages "
                `LB.append` btnPressedLBS
                `LB.append` " times"
        bodyLBS =
            "{\"callback_query_id\": \""
                `LB.append` queryIDLBS
                `LB.append` "\", \"text\": \""
                `LB.append` msgLBS
                `LB.append` "\"}"
        reqWithHeaders = setRequestHeaders
